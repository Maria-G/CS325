...
public enum CopyOfLocation {
	 ...
	 */
	public static int distance(CopyOfLocation from, CopyOfLocation to) {
		if (to == CopyOfLocation.R_BEAR_OFF) {
			// R1 == 1; R2 == 2, etc.
			// Thus the distance from bear off to R2 is -2
			// as red moves away from black inner table
			return -from.ordinal();
		}
		if (to == CopyOfLocation.B_BEAR_OFF) {
	 ...
	 *         and a distance.
	 */
	public static CopyOfLocation findLocation(Color p, CopyOfLocation from, int distance) {
		int _to;
		if (from == CopyOfLocation.B_BAR) {
			_to = distance;
		} else if (from == CopyOfLocation.R_BAR) {
		...
		if (_to <= 0) {
			_to = CopyOfLocation.R_BEAR_OFF.ordinal();
		} else if (_to >= N_LOCATIONS) {
			_to = CopyOfLocation.B_BEAR_OFF.ordinal();
		}
		CopyOfLocation to = values()[_to];
		return to;
	}
}
